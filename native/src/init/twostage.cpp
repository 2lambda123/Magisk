...
void FirstStageInit::prepare() {
  ...
  if (struct stat st{}; fstatat(-1, "/sdcard", &st, AT_SYMLINK_NOFOLLOW) != 0 &&
                        fstatat(-1, "/first_stage_ramdisk/sdcard", &st,
                                AT_SYMLINK_NOFOLLOW) != 0) {
    if (config->force_normal_boot) {
      xmkdirs("/first_stage_ramdisk/storage/self", 0755);
      xsymlink("/system/system/bin/init",
               "/first_stage_ramdisk/storage/self/primary");
      LOGD("Symlink /first_stage_ramdisk/storage/self/primary -> "
           "/system/system/bin/init\n");
      close(xopen("/first_stage_ramdisk/sdcard", O_RDONLY | O_CREAT | O_CLOEXEC,
                  0));
    } else {
...
bool SecondStageInit::prepare() {
  ...
  if (sfs.f_type == RAMFS_MAGIC || sfs.f_type == TMPFS_MAGIC) {
    // We are still on rootfs, so make sure we will execute the init of the 2nd
    // stage
    unlink("/init");
