...
static string extract_quoted_str_until(chars<escapes...>, chars<breaks...>,
  ...
  for (size_t cur = pos;; ++cur) {
    ...
    if (cur == string_view::npos || ((str[cur] == breaks) || ...) ||
...
static kv_pairs parse_impl(chars<padding...>, string_view str) {
  ...
  bool quoted = false;
  for (size_t pos = 0u; pos < str.size();
       pos = str.find_first_not_of(delim, pos)) {
    auto key = extract_quoted_str_until(chars<padding..., delim>{},
                                        chars<'='>{}, str, pos, quoted);
    ...
    auto value =
        extract_quoted_str_until(chars<delim>{}, chars<>{}, str, pos, quoted);
...
void load_kernel_info(BootConfig *config) {
  ...
  char file_name[128];
  read_dt("fstab_suffix", fstab_suffix) read_dt("hardware", hardware)
