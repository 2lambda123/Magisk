...
static void dump_preload() {
  int fd =
      xopen("/dev/preload.so", O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, 0644);
  if (fd < 0)
    return;
  fd_channel ch(fd);
...
bool MagiskInit::hijack_sepolicy() {
  ...
  if (access("/system/bin/init", F_OK) == 0) {
    ...
    // This meant that instead of going through convoluted methods trying to
    // alter and block init's control flow, we can just LD_PRELOAD and replace
    // the security_load_policy function with our own implementation.
    dump_preload();
  ...
  if (access(SELINUX_ENFORCE, F_OK) != 0) {
    ...
    mount_list.erase(std::remove_if(mount_list.begin(), mount_list.end(),
                                    [](const string &s) {
                                      return s == "/proc" || s == "/sys";
                                    }),
                     mount_list.end());
  ...
  if (!dt_compat.empty()) {
    ...
    // The only purpose here is actually to wait for init to mount selinuxfs for
    // us
  ...
  if (auto sepol = unique_ptr<sepolicy>(sepolicy::from_file(MOCK_LOAD));
      sepol) {
    sepol->magisk_rules();
  ...
  // Write to the enforce node ONLY after sepolicy is loaded. We need to make
  // sure the actual init process is blocked until sepolicy is loaded, or else
  // restorecon will fail and re-exec won't change context, causing boot
  // failure. We (ab)use the fact that init reads the enforce node, and because
